...
# MCP Server (JSON-RPC initialize over HTTP/SSE)
curl -i -X POST http://localhost:8051/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","method":"initialize","params":{"protocolVersion":"1.0"},"id":1}'
...
- **MCP Connection**: Use `http://localhost:8051/mcp` in your IDE (or `https://archon.v1su4.com/mcp` in production)
